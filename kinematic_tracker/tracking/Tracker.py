...
from kinematic_tracker.tracking.TrackedInstance import TrackedInstance
...
class Tracker(object):
    ...
    def __init__(self, imageSize: Tuple[int, int]=(1280, 720), similarityThreshold: float=0.9, iouThreshold: float=0.1, invalidationTimeMs: int = 5000,
                 minVisiblePoses: int = 4, relativeOrder:bool = True, minHistory: int = 5):
    ...
    def GetBoundingBoxes(self) -> List[BoundingBoxBase]:
        ...
        for instance in self.trackedInstances:
            # introduce min history count to avoid single fals predictions
            if len(instance.history) > self.minHistory:
    ...
    def GetNotKnownInstances(self, boundingBoxes: List[BoundingBoxBase], iouThreshold: Optional[float] = None,
                             ...
                             histWeightFactor:float = 1.0, weighted: bool = True) -> List[BoundingBoxBase]:
        ret = []
    ...
    def AddSupervision(self, poseModel: List[PoseModelBase], autoCheckInvalidation: bool = True) -> List[float]:
        ...
        for model in poseModel:
            ...
            trackInfos = TrackingInformation(model.kinematicChain, self.imageSize)
    ...
    def Predict2DBB(self, maxHistoryCount: int = 5, weighted: bool = True,
                    weigthFactor: float = 1.0, weightMethod: str = "expo") -> List[BoundingBoxBase]:
        #calculate mean change of the bounding box over the known history
        resBB = []
        pointsOfInterest = [0, 3]
        for instance in self.trackedInstances:
            history = instance.history.GetLastItems(maxHistoryCount, delete=False)
            if history is not None and history.__len__() >= maxHistoryCount:
                # create to track the 2 Edge points of an bounding box
                flows: List[List[OpticalFlow]] = [list() for i in range(0, len(pointsOfInterest))]
                # get weigths for histroy
                flowWeigths = self.GetPredictionWeigths(maxHistoryCount-1, history.__len__(), weigthFactor,
                                                        weightMethod=weightMethod)

                #print(weightMethod + "factor: " + str(weigthFactor) + str(flowWeigths))

                maxSpeed = 0.0
                latestHist = history[-1]
                latestBB: BoundingBoxBase = latestHist.descriptor.kinematicChain.GetBoundingBox()
                if self.relativeOrder:
                    for i in range(0, history.__len__()-1):
                        ...
                        for j in range(0, len(pointsOfInterest)):
                            point = pointsOfInterest[j]
                            flows[j].append(edgesBB1[point].CalculateFlow(edgesBB2[point]))
                ...
                for j in range(0, flows.__len__()):
                    if weighted:
                        meanFlows.append(OpticalFlow.Mean(flows[j], flowWeigths))
                    else:
                ...
                edgesOfInterest = []
                for j in range(0, len(pointsOfInterest)):
                    point = movedBBEdges[pointsOfInterest[j]]
                    disp = meanFlows[j].Displacement()
                    point += disp
                    edgesOfInterest.append(point)

                resBB.append(latestBB.CreateBoundingBox(keyPoints=edgesOfInterest))
        return resBB
    ...
    def GetPredictionWeigths(self, maxHistoryCount: int, histLength: int, weigthFactor: float,
        ...
        if weightMethod == "expo":
            for x in range(0, maxHistoryCount, 1):
                weight = math.exp(-weigthFactor * x)
            ...
            if history is not None and history.__len__() > 1:
                ...
                if self.relativeOrder:
                    for i in range(0, compLength):
                        ...
                        for j in range(0, flows.__len__()):
                            meanFlows[j].append(copy.deepcopy(flows[j]))
                ...
                for i in range(0, meanFlow.__len__()):
                    ...
                    movedPose = type(instance.trackingInfos.kinematicChain.poses[0])(moved.trans, moved.rotation)
